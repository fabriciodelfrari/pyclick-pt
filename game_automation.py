...
class GameBot:
    def __init__(self):
        ...
        self.periodic_flow_interval = 600  # 10 minutos
    ...
    def find_potion_in_inventory(self, potion_type):
        """
        Encontra uma poção específica no inventário
        """
        try:
            ...
            # Definir máscara de cor baseada no tipo de poção
            if potion_type == 'red_potion':
                mask = cv2.inRange(hsv, np.array([0, 50, 50]), np.array([20, 255, 255]))
            elif potion_type == 'green_potion':
                mask = cv2.inRange(hsv, np.array([35, 50, 50]), np.array([85, 255, 255]))
            ...
            if contours:
                # Pegar o maior contorno (provavelmente a poção)
                largest_contour = max(contours, key=cv2.contourArea)
                x, y, w, h = cv2.boundingRect(largest_contour)
                
                # Converter coordenadas para posição absoluta na tela
                abs_x = self.inventory_region[0] + x + w // 2
                abs_y = self.inventory_region[1] + y + h // 2
                
                # Salvar imagem de debug se necessário
                if self.debug_mode:
                    debug_img = inv_region.copy()
                    cv2.rectangle(debug_img, (x, y), (x + w, y + h), (0, 255, 0), 2)
                    cv2.imwrite(f'debug_potion_{potion_type}.png', debug_img)
            ...
            logging.error(f"Erro ao encontrar poção {potion_type}: {e}")
            return None
    ...
    def background_service(self):
        ...
        while self.running:
            try:
                ...
                # Fluxo periódico a cada 10 minutos
                if current_time - self.last_periodic_flow >= self.periodic_flow_interval:
                    if self.show_action_logs:
                        logging.info("Iniciando fluxo periódico F2->F3->F1...")
                    
                    # Pressionar F2
                    pyautogui.press('f2')
                    time.sleep(1)
                    
                    # 3 cliques direitos com intervalo de 5s cada
                    for i in range(3):
                        pyautogui.rightClick()
                        if i < 2:  # Não dar delay após o último
                            time.sleep(5)
                    ...
                    if self.show_action_logs:
                        logging.info("Fluxo periódico F2->F3->F1 concluído")
    ...
    def calibrate_regions(self):
        ...
        print("- [Shift +/-] - Aumentar/diminuir altura")
        ...
        try:
            ...
            while True:
                ...
                instructions = [
                    ...
                    "1-7: Selecionar | WASD: Mover | +/-: Redimensionar",
                    "SPACE: Salvar | Q: Sair"
                ]
...
def main():
    ...
    while True:
        ...
        print("7. TESTE DE SLOT VAZIO (usar poção no jogo)")
        print("8. Configurar logs")
        print("9. Sair")
